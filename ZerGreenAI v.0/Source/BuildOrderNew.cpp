...
std::vector<BWAPI::UnitType> ZerGreenAI::protossBuildingTypes = { UnitTypes::Protoss_Arbiter_Tribunal, UnitTypes::Protoss_Assimilator, UnitTypes::Protoss_Citadel_of_Adun, UnitTypes::Protoss_Cybernetics_Core, UnitTypes::Protoss_Fleet_Beacon, UnitTypes::Protoss_Forge, UnitTypes::Protoss_Gateway, UnitTypes::Protoss_Nexus, UnitTypes::Protoss_Observatory, UnitTypes::Protoss_Photon_Cannon, UnitTypes::Protoss_Pylon, UnitTypes::Protoss_Robotics_Facility, UnitTypes::Protoss_Shield_Battery, UnitTypes::Protoss_Stargate, UnitTypes::Protoss_Templar_Archives };
...
std::string ZerGreenAI::BuildOrderManager::getSaveFileName(int i)
{
	return "bwapi-data/read/newBuildOrderGames/game" + std::to_string(i) + ".bo";
}

void ZerGreenAI::BuildOrderManager::onStart()
{
	std::ifstream gameNumberFileInput("bwapi-data/read/newBuildOrderGames/gamenumber.txt", ifstream::in);
	...
	std::ofstream gameNumberFileOutput("bwapi-data/read/newBuildOrderGames/gamenumber.txt", ifstream::out | ifstream::trunc);
...
auto getNeuralNetworkInputStream()
{
	return [&]() {
#ifndef SSCAIT
		std::ifstream model("bwapi-data/read/newBuildOrderManager.txt");
...
auto getNeuralNetworkOutputStream()
{
	return [&]() {
#ifndef SSCAIT
		std::ofstream model("bwapi-data/read/newBuildOrderManager.txt");
		assert(model.is_open());
		return model;
...
std::vector<float> BuildOrderManager::getInput(UnitType whatAction)
{
	...
	for (UnitType t : protossBuildingTypes)
	{
...
UnitType ZerGreenAI::BuildOrderManager::chooseNextAction(float chanceToChooseRandomly)
{
	if ((float)rand() / (float)RAND_MAX < chanceToChooseRandomly)
	{
		...
		return protossBuildingTypes[rand() % protossBuildingTypes.size()];
	}
		
	float bestValue = std::numeric_limits<float>::lowest();
	UnitType bestAction = UnitTypes::None;
	for (UnitType testAction : protossBuildingTypes)
	{
...
void ZerGreenAI::BuildOrderManager::onUnitMorph(Unit u)
{
	...
	{
		// TODO: Fix bug, when enemy geyser destroyed, will decrement
		numUnitsOfType[UnitTypes::Protoss_Assimilator]--;
	}
}
