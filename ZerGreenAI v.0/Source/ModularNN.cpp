...
template<>
std::string ModularNN::StandardLayer<ModularNN::ActivationFunction::ReLU>::toString() {
	return "ReLU" + weightsToString();
}

template<ModularNN::ActivationFunction actFunc>
void ModularNN::StandardLayer<actFunc>::updateWeights(fv input, fv correctOutput, LayerIterator remainingLayersBegin, LayerIterator remainingLayersEnd, float learningSpeed)
{
	for (unsigned i = 0; i < (inputSize + 1) * outputSize; i++)
	{
		fv initialOutput = run(input);
		initialOutput = runLayers(initialOutput, remainingLayersBegin, remainingLayersEnd);

		float oldWeight = weights[i];
		weights[i] += learningSpeed * ((((float)rand() / (float)RAND_MAX) *2) - 1);
		fv newOutput = run(input);
		newOutput = runLayers(newOutput, remainingLayersBegin, remainingLayersEnd);
		if (vectorDistance(newOutput, correctOutput) < vectorDistance(initialOutput,correctOutput))
		{
			continue;
		}
		else
...
template<>
std::string ModularNN::StandardLayer<ModularNN::ActivationFunction::Sigmoid>::toString() {
	return "Sigmoid" + weightsToString();
}

template<>
std::string ModularNN::StandardLayer<ModularNN::ActivationFunction::Softmax>::toString() {
	return "Softmax" + weightsToString();
}

template<>
std::string ModularNN::StandardLayer<ModularNN::ActivationFunction::Tanh>::toString() {
	return "Tanh" + weightsToString();
}
...
template<ModularNN::ActivationFunction actFunc>
std::string ModularNN::StandardLayer<actFunc>::weightsToString()
{
	std::string returnValue;
	returnValue += " ";
	returnValue += std::to_string(inputSize);
	returnValue += " ";
	returnValue += std::to_string(outputSize);
	for (auto i = 0u; i < inputSize + 1; ++i)
		for (auto o = 0u; o < outputSize; ++o)
		{
			returnValue += " ";
			returnValue += std::to_string(weights[i * outputSize + o]);
		}
	returnValue += "\n";
	return returnValue;
}
...
fv ModularNN::runLayers(fv input, LayerIterator begin, LayerIterator end)
{
	for (; begin != end;begin++)
	{
		input = (*begin)->run(input);
	}
	return input;
}

// Doesn't use runLayers because constructing the subset of layers may (or may not, idk) impact the performance. Perhaps I shouldn't care about that too much idk.
fv ModularNN::runUpToLayerExclusive(fv input, LayerIterator whatLayer)
{
	for (auto iter = layers.begin(); iter != whatLayer; iter++)
	{
		input = (*iter)->run(input);
	}
	return input;
}

void ModularNN::updateNeuralNetwork(fv input, fv correctOutput, float learningSpeed)
{
	for (LayerIterator layer = layers.begin(); layer != layers.end(); layer++)
	{
		(*layer)->updateWeights(runUpToLayerExclusive(input, layer), correctOutput, layer + 1, layers.end(), learningSpeed);
	}
}

float ModularNN::vectorDistance(fv a, fv b)
{
	float sum = 0;
